...
include('../dbConnection.php');
 ...
 if(isset($_REQUEST['courseSubmitBtn'])){
  ...
  if(isset($_FILES['course_img']) && $_FILES['course_img']['error'] === UPLOAD_ERR_OK){
    ...
    if(move_uploaded_file($course_image_temp, $img_folder)){
      try {
        ...
        if ($stmt->execute()) {
          ...
          $msg = '<div class="alert alert-danger col-sm-6 ml-5 mt-2" role="alert"> Unable to Add Course: ' . $stmt->error . '</div>';
        }
    ...
    $msg = '<div class="alert alert-danger col-sm-6 ml-5 mt-2" role="alert"> Please select a valid image file </div>';
  }
  }
  }
 ?>
